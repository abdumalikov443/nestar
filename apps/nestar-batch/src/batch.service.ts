...
export class BatchService {
  ...
  public async batchTopAgents(): Promise<void> {//@ts-ignore
    const agents: Member[] = this.memberModel
      .find({
    ...
    const promisedList = agents.map(async (ele: Member) => {
      ...
      return await this.memberModel.findOneAndUpdate(_id, { propertyRank: rank });
    });
    await Promise.all(promisedList);
  }
